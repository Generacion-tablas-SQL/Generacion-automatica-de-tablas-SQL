...
def main():
    create_table = "CREATE TABLE Persona (" \
                   ...
                   "ent INT unique CHECK (INT > 25 AND INT < 45)," \
                   "string VARCHAR(15) NOT NULL CHECK (string LIKE 'C%' and LENGTH(string) > 5 and LENGTH(string) < 20)," \
                   "fec1 DATE NOT NULL, " \
                   "fec2 TIMESTAMP(2) NOT NULL)"

    select1 = "SELECT string FROM Persona WHERE string like 'Car'"
    select2 = "SELECT ent, real FROM Persona WHERE ent > 30 and real = 0.01"
    select3 = "SELECT ent, string FROM Persona WHERE ent > 30 and string = 'Carmen'"
    ...
    try:
        print(*poblador_tablas(create_table, select5), sep='\n')
